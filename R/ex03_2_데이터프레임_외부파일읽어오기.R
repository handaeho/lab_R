...
# 엑셀파일 첫 번째 행이 변수명이 아니라면?
# 데이터프레임이름 <- read_excel("엑셀파일경로/엑셀파일명.xlsx", col_name = F)
df_exam_novar <- read_excel("data_excel/excel_exam.xlsx", col_names = F) 
df_exam_novar

# 엑셀파일에 시트가 여러개라면?
# 데이터프레임이름 <- read_excel("엑셀파일경로/엑셀파일명.xlsx", sheet = 시트개수)
df_exam_sheet <- read_excel("data_excel/excel_exam.xlsx", sheet = 3) 
df_exam_sheet
